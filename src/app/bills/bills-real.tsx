          ...
          {per.map((p) => (
            <div key={p.acc.id}>
              ...
              <div className="relative h-9 rounded-lg bg-white/[0.03]">
                ...
                <Link href="/transactions" className="absolute top-1 h-7 rounded-md grid place-items-center text-[11px] text-white" style={{ left: posOf(p.cyc.start), width: widthOf(p.cyc.start, p.cyc.end), background: "linear-gradient(135deg,#7C3AED,#D7B8F3)", opacity: 0.9 }} title="Ciclo atual">
          ...
          return (
            <Card key={p.acc.id}>
              ...
              <div className="mt-2 flex items-center gap-2 text-[10px] text-muted num"><span>{ddmm(p.cyc.start)}</span>
                <div className="flex-1 h-1.5 rounded-full bg-white/[0.06] overflow-hidden"><div className="h-full rounded-full" style={{ width: `${prog}%`, background: "linear-gradient(90deg,#7C3AED,#D7B8F3)" }} /></div>
...
function CycleEditor({ accountId, closing, due, onSaved }: { accountId: string; closing: number; due: number; onSaved: () => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <button onClick={save} disabled={busy} className="grid place-items-center h-7 w-7 rounded-lg text-white" style={{ background: "linear-gradient(135deg,#7C3AED,#D7B8F3)" }}>{busy ? <Loader2 size={12} className="animate-spin" /> : <Check size={13} />}</button>
    </div>
  );
}
